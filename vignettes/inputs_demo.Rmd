---
title: "Using Input Variables"
author: "MCSimMod Development Team"
date: "`r Sys.Date()`"
output: html_vignette
vignette: >
  %\VignetteIndexEntry{Using Input Variables}
...
The **MCSimMod** package allows one to solve initial value problems for ordinary differential equation (ODE) models that include input variables. These are variables that may vary in time, but which are not state variables and which are independent of other model variables (including parameters). The authors of the **deSolve** package use the term "forcing function" to such input variables. The **MCSimMod** package uses functions from the **deSolve** package to solve ODE initial value problems, and so we use **deSolve** forcing funtions to implement input variables. One can learn more about **deSolve** forcing functions in the [deSolve package documentation](https://cran.r-project.org/web/packages/deSolve/deSolve.pdf), but here we will illustrate how to perform simulations with a model that includes a single input variable.

## A Classical Pharmacokinetic Model

To demonstrate events, we will use the classical pharmacokinetic (PK) model illustrated in the figure below.
...
Using the following commands, we create a model object (i.e., an instance of the `MCSimMod` class) using the model specification file `pk1.model` that is included in the **MCSimMod** package.
```{r, results='hide'}
...
pk1_mod = Model(mName=pk1_mod_name)
```
...
# Run simulation.
out = pk1_mod$runModel(times, forcing=list(data=M_table))
```
...
# Run simulation.
out2 = pk1_mod$runModel(times, forcing=list(data=M_table2))
```
...
plot(out[, "time"], out[, "C"], type="l", lty=1, lwd=2, xlab="Time (h)",
     ylab="Concentration (mg/L)", ylim=c(0, 2500))
lines(out2[, "time"], out2[, "C"], lty=2, lwd=2)
legend("topright", legend=c("Varying Body Mass", "Constant Body Mass"),
       lty=c(1, 2), lwd=2)
```
