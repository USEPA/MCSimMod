---
title: "Implementing an Exponential Model"
author: "MCSimMod Development Team"
date: "`r Sys.Date()`"
output: html_vignette
vignette: >
  %\VignetteIndexEntry{exponential_demo}
...
Next, we will use the **MCSimMod** package to define an ODE model and solve an initial value problem. Solving an initial value problem is synonymous with "performing a simulation" with the model.
...
Using the following commands, we create an exponential model object (i.e., an instance of the `MCSimMod` class) using the model specification file `exponential.model` that is included in the **MCSimMod** package.
```{r, results='hide'}
...
```{r, results='hide'}
# Change the values of the model parameters from there default values: set the 
...
We can remove output files that were created when building the model (i.e., files with names ending in ".o", ".c", "_inits.R", and ".dll" or ".so") by calling the `cleanup` method.
```{r, results='hide'}
