---
title: "(6) A Classical Pharmacokinetic Model"
author: "Dustin Kapraun"
date: "`r Sys.Date()`"
output: html_vignette
vignette: >
  %\VignetteIndexEntry{(6) A Classical Pharmacokinetic Model}
...
We used the [GNU MCSim](https://www.gnu.org/software/mcsim/) model specification language to implement the classical PK model. The complete MCSim model specification file for this model, `pk1.model`, can be found in the `extdata` subdirectory of the **MCSimMod** package installation directory.
...
mod_path <- normalizePath(file.path(system.file(package = "MCSimMod"), "extdata"))
...
Note that executing the command `pk1_mod$updateParms(c(k01=0.8, k12=0.4, Vd=45, A0_init=200))` updates the parameter values (replacing the default values that were provided in the model specification file) and that executing the command `pk1_mod$updateY0()` updates the initial values of the state variables. (We do not need to provide values for the parameters that correspond to the initial values of the state variables other than $A_0$, as the default values of zero for those state variables are appropriate for this scenario.)
...
pk1_mod$Y0
```
(Note that $k_{01}$ was assigned its default value of 1.0 because it wasn't specifically provided a value in the call to the `updateParms()` method.)
