---
title: "Implementing a Predator=Prey Model"
author: "MCSimMod Development Team"
date: "`r Sys.Date()`"
output: html_vignette
vignette: >
  %\VignetteIndexEntry{Implementing a Predator=Prey Model}
...
## A Mathematical Model for Prey and Predator Population Dynamics
...
Using the following commands, we create a model object (i.e., an instance of the `MCSimMod` class) using the model specification file `pred_prey.model` that is included in the **MCSimMod** package.
```{r, results='hide'}
...
pp_mod = Model(mName=pp_mod_name)
```
...
legend("topright", c("Rabbits", "Foxes"), lty=c(1, 2),
       lwd=2)
```
