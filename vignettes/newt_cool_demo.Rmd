---
title: "Implementing a Model for Newton's Law of Cooling"
author: "MCSimMod Development Team"
date: "`r Sys.Date()`"
output: html_vignette
vignette: >
  %\VignetteIndexEntry{Implementing a Model for Newton's Law of Cooling}
...
## A Mathematical Model for Newton's Law of Cooling
...
Using the following commands, we create a model object (i.e., an instance of the `MCSimMod` class) using the model specification file `newt_cool.model` that is included in the **MCSimMod** package.
```{r, results='hide'}
...
newt_mod = Model(mName=newt_mod_name)
```
...
Note that executing the command `newt_mod$updateParms(c(T0=95, Tamb=22, r=0.7))` updated the parameter values (replacing the default values that were provided in the model specification file) and that executing the command `next_mod$updateY0()` updated the initial value of the state variable `T` using the updated value of the parameter `T0`.
...
plot(out[, "time"], out[, "T"], type="l", lty=1, lwd=2,
     xlab="Time", ylab="Temperature (C)", ylim=c(0, 100))
abline(h=newt_mod$parms[["Tamb"]], lty=2, lwd=2)
legend("topright", c("Tea", "Ambient Air"), lty=c(1, 2),
       lwd=c(2, 2))
```
