...
The **MCSimMod** package facilitates ordinary differential equation (ODE) modeling within the R environment. **MCSimMod** allows one to solve initial value problems (IVPs) for ODE models described in the [MCSim](https://www.gnu.org/software/mcsim/) model specification language using the R package **deSolve**. This system enables users to take advantage of the flexibility and post hoc data analysis capabilities of the interpreted language R while also achieving computational speeds typically associated with compiled programming languages like C and FORTRAN. Furthermore, this system encourages modelers to use separate files for defining models and executing model simulations, which can, in turn, improve modularity and reusability of source code developed for modeling analyses.
...
One can create a new **Model** object using the function `createModel()`. It is necessary to supply (as an argument to the `createModel()` function) either a string or the name of a file that contains the model specification text. For example, if `mod_string` is a character string containing model specification text, one could use the following command to create a `Model` object from that string.
